...
export class PricingService {
  ...
  calculateMachineHourlyCost(printer: Printer, settings: Settings): number {
    const energyCost =
      (printer.power_consumption_watts / 1000) *
      Number(settings.electricity_cost_kwh);
    const depreciation =
      Number(printer.purchase_price) / printer.lifespan_hours;
  ...
  calculateOrderPricing(
    ...
    const prepTime =
      preparationTimeMinutes || product.preparation_time_minutes || 0;
    const laborCost = (prepTime / 60) * Number(settings.labor_rate_hourly);

    // 4. Extras
    const addonsCost = product.addons.reduce(
      (sum, item) => sum + Number(item.cost_unit),
      0,
    );
